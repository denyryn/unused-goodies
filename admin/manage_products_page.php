<?php  
    include ("./php/manage_products.php");
    include ("./php/img_upload.php");
...
<body class="font-rubik ">
    <div class="p-4 sm:ml-64">
        <div class="flex flex-row items-center justify-between h-full p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            ...
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" method="post" enctype="multipart/form-data">
                <table class="border-separate border-spacing-y-4" border="1"  style="border-spacing: 0 10px;">
                    <tr>
                        ...
                        <td><input class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block p-2.5" type="text" id="product_name" name="product_name" required></td>
                    </tr>
                    <tr>
                        <td><label for="description">Description</label></td>
                        <td>:</td>
                        <td><textarea class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-sm border border-gray-300 focus:ring-blue-500 focus:border-blue-500" id="description" name="description" cols="30" required></textarea></td>
                    </tr>
                    <tr>
                        <td><label for="price">Price</label></td>
                        <td>:</td>
                        <td><input class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block p-2.5" type="text" id="price" name="price" required></td>
                    </tr>
                    <tr>
                        <td><label for="stock_quantity">Stock Quantity</label></td>
                        <td>:</td>
                        <td><input class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block p-2.5" type="number" id="stock_quantity" name="stock_quantity" required></td>
                    </tr>
                    <tr>
                        <td><label for="category_id">Category</label></td>
                        <td>:</td>
                        <td>
                        <select class="block p-2 text-gray-900 border border-gray-300 rounded-sm cursor-pointer ps-2 pe-2 bg-gray-50 focus:ring-blue-500 focus:border-blue-500" id="category_id" name="category_id" required>
                    ...
                    <tr>
                        ...
                        <td>:</td>
                        <td>
                            <label class="block mb-1 text-sm font-medium text-gray-900" for="image">Upload an Image</label>
                            <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-sm cursor-pointer bg-gray-50 " type="file" id="image" name="image" accept="image/*" required>
                            <p class="mt-1 text-sm text-gray-500" id="image_input_help">PNG, JPG or JPEG (PREF. 1080x1080px).</p>

                        </td>
                    </tr>
                </table>
                <div>
                    <input class="mt-4 text-white bg-black btn hover:bg-white hover:text-black hover:border-black" type="submit" name="add_product" value="Submit">
                </div>
            </form>
        </div>
    </div>
                    ...
                    <tbody>
                        <?php foreach ($products as $product) : ?>
                            <tr>
                                ...
                                <td >
                                    <a class="hover:underline" href="./edit_products.php?action=edit&product_id=<?php echo $product['product_id']; ?>">Edit</a>
                                    |
                                    <a class="text-red-400 transition-colors hover:underline hover:text-red-700" href="?action=delete&product_id=<?php echo $product['product_id']; ?>" onclick="return confirm('Are you sure you want to delete this product?')">Delete</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
