<?php
    ...
    include("./php/manage_categories.php");
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'edit' && isset($_GET['category_id'])) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            if (isset($_POST['update_category'])) {
                ...
                $targetDir = "../uploads/category_images/base/$category_name/";
                $hoverTargetDir = "../uploads/category_images/hover/$category_name/";
                ...
                if (updateCategory($category_id, $category_name, $category_description, $imagePath, $imageHoverPath)) {
...
<body class="font-rubik">
    ...
    <div class="p-4 sm:ml-64">
        <div class="h-full p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            ...
            <!-- Edit Category Form -->
            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'] . '?action=edit&category_id=' . $category_id); ?>" method="post" enctype="multipart/form-data">
                ...
                <table class="border-separate border-spacing-y-4" border="1"  style="border-spacing: 0 10px;">
                    <tr>
                        ...
                        <td><input class="w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block p-2.5" type="text" id="category_name" name="category_name" value="<?php echo $category['category_name']; ?>" required></td>
                    </tr>
                    <tr>
                        <td><label for="category_description">Description</label></td>
                        <td>:</td>
                        <td><textarea class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-sm border border-gray-300 focus:ring-blue-500 focus:border-blue-500" id="category_description" name="category_description" required><?php echo $category['category_description']; ?></textarea></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td></td>
                        <td>
                            <?php
                                if ($category['image_base_path'] !== null) {
                                    echo '<img class="object-cover w-32 aspect-square" src="' . $category['image_base_path'] . '" alt="image_base">';
                                } else {
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-sm cursor-pointer bg-gray-50 " type="file" id="image_base_path" name="image_base_path" accept="image/*">
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                                if ($category['image_hover_path'] !== null) {
                                    echo '<img class="object-cover w-32 aspect-square" src="' . $category['image_hover_path'] . '" alt="">';
                                } else {
                                    echo '<p class="text-red-500">No Photo Currently Available.</p>';
                                }
                            ?>
                    ...
                    <tr>
                        ...
                        <td>
                            <label class="block mb-1 text-sm font-medium text-gray-900" for="image">Upload an Image</label>
                            <input class="block w-full text-sm text-gray-900 border border-gray-300 rounded-sm cursor-pointer bg-gray-50 " type="file" id="image_hover_path" name="image_hover_path" accept="image/*">
                            <p class="mt-1 text-sm text-gray-500" id="image_input_help">PNG, JPG or JPEG (PREF. 1080x1080px).</p>
                        </td>
                    </tr>
                </table>
                <div>
                    <input class="mt-4 text-white bg-black btn hover:bg-white hover:text-black hover:border-black" type="submit" name="update_category" value="Update Category">
                </div>
            </form>
        </div>
    </div>
