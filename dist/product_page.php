    ...
    <div class="max-w-2xl px-4 py-16 mx-auto sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
        ...
        ?>
            ...
            <!-- <a href="detailed_product_page.php?product_id=<?php echo $product['product_id']; ?>">
                <div class="max-w-[300px] bg-white border transition duration-200 ease-in-out active:scale-95 border-gray-200 rounded-sm shadow-sm w-fit dark:bg-gray-800 dark:border-gray-700 m-3 cursor-pointer">
                    <div class="cursor-pointer aspect-square">
                        <img class="object-cover w-full h-full group-hover:scale-110" src="<?php echo $product['image_path']; ?>" 
                        alt="<?php $product['product_name']?>">
                    </div>
                    <div class="px-5 pb-5">
                        <h5 class="my-3 font-normal tracking-wide text-gray-900 text-md ps-0 dark:text-white"><?php echo $product['product_name']; ?></h5>
                        <div class="flex items-center justify-between">
                            <span class="font-semibold text-gray-900 text-md dark:text-white">Rp <?php echo number_format($product['price'], 2, ',', '.'); ?></span>
                        </div>
                    </div>
                </div>
            </a> -->

            <a href="detailed_product_page.php?product_id=<?php echo $product['product_id']; ?>" class="p-3 transition duration-200 ease-in-out border rounded-md group active:scale-95">
                <div class="w-full overflow-hidden bg-gray-200 rounded-md aspect-h-1 aspect-w-1 aspect-square xl:aspect-h-8 xl:aspect-w-7">
                <img src="<?php echo $product['image_path']; ?>" alt="<?php $product['product_name']?>." class="object-cover object-center w-full h-full group-hover:opacity-75">
                </div>
                <h3 class="mt-4 text-sm text-gray-700"><?php echo $product['product_name']; ?></h3>
                <p class="mt-1 text-lg font-medium text-gray-900">Rp <?php echo number_format($product['price'], 2, ',', '.'); ?></p>
            </a>
